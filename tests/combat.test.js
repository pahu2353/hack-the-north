...
function bench(t) {
  ...
  const shooter = game.units[0];
  const enemies = game.units.filter(u => u.team !== shooter.team);
...
test('crosshair changes the target and boosts only the aligned enemy, without bypassing reaction time', t => {
  const { game, shooter, target, enemies, place, aim, advance } = bench(t);
  place(target, 20, 5); // closer, but outside the crosshair
  const aligned = enemies[1]; aligned.alive = true; place(aligned, 25, 3);
...
test('crosshair targeting respects walls and range, and selects only the nearest aligned enemy', t => {
  const { game, shooter, target, enemies, place, aim } = bench(t);
  place(shooter, 10, 30);
  ...
  assert.equal(target.hp, MAX_HP);
  place(target, 20, 3);
  const farther = enemies[1]; farther.alive = true; place(farther, 25, 3);
  shooter.cooldown = 0;
  aim(); stepGame(game, STEP);
  assert.equal(target.hp, MAX_HP - RIFLE.damage);
