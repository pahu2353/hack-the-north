...
import { aimFist, aimOrStep, fourFingers, pointDirection, repeatDelay, thumbDirection } from '../public/commander/gestures.js';
...
test('a sideways thumb still reads as a direction, for whatever wants it next', () => {
...
test('an upright thumb is not read as a sideways one', () => {
...
test('a held sign steps faster the longer it is held, down to a floor', () => {
  ...
  // Five agents: holding through the whole squad takes well under two and a half seconds.
  assert(delays.slice(0, 4).reduce((a, b) => a + b, 0) < 2500);
});
...
// Four fingers up, thumb folded in: the sign for the next agent.
function fourHand(degrees = 0) {
  const put = point => rotate(degrees, point);
  const hand = Array.from({ length: 21 }, () => put([0.03, -0.10]));
  hand[0] = put([0, 0]);
  hand[9] = put([0.01, -0.20]);
  hand[2] = put([-0.05, -0.10]);
  hand[3] = put([-0.06, -0.14]);
  hand[4] = put([-0.04, -0.15]); // thumb folded across the palm
  for (const [i, [tip, pip]] of [[8, 6], [12, 10], [16, 14], [20, 18]].entries()) {
    hand[pip] = put([0.03 + i * 0.03, -0.17]);
    hand[tip] = put([0.03 + i * 0.03, -0.30]); // well past the knuckle: extended
  }
  return hand;
}

test('four fingers up are the sign for the next agent', () => {
  for (const degrees of [0, 20, -20]) assert(fourFingers(fourHand(degrees)) > 0, `${degrees}\u00b0`);
});

test('a fist, one finger and an open palm are not four fingers', () => {
  assert.equal(fourFingers(fistHand(0)), 0);
  assert.equal(fourFingers(pointingHand(0)), 0);
  // An open palm has the thumb out as well, and is the one shape closest to this sign: a hand
  // waved around must not step through the squad.
  const palm = fourHand(0);
  palm[4] = { x: palm[0].x - 0.18, y: palm[0].y - 0.20 };
  assert.equal(fourFingers(palm), 0);
});

test('four fingers change agent immediately, even straight after aiming', () => {
  // Mid-aim: the fist owns the hand.
  assert.deepEqual(aimOrStep(1, 0, 0), { steering: true, step: false });
  // Tracking blinked and the fist was missed for a frame: the aim is not handed over.
  assert.deepEqual(aimOrStep(0, 0, 120), { steering: true, step: false });
  // Four fingers win at once, without waiting for the fist's grace period to run down.
  assert.deepEqual(aimOrStep(0, 1, 0), { steering: false, step: true });
  assert.deepEqual(aimOrStep(1, 1, 0), { steering: false, step: true });
  // Hand lowered long enough: back to normal.
  assert.deepEqual(aimOrStep(0, 0, 900), { steering: false, step: false });
});
