...
export function createGame({ defenders = 'bots' } = {}) {
  ...
  TEAMS.attack.names.forEach((name, i) => game.units.push(makeAgent(game, 'attack', name, map.spawns.attack[i])));
  map.spawns.defend.forEach((post, i) => {
    if (defenders === 'bots') {
      game.units.push(makeUnit(game, {
        team: 'defend', kind: 'bot', name: `E${i + 1}`, x: post.x, y: post.y, post, r: 0.6, hp: 100, maxHp: 100,
        speed: 4.5, reaction: 0.28 + Math.random() * 0.12, facing: Math.PI / 2,
      }));
    } else {
      game.units.push(makeAgent(game, 'defend', TEAMS.defend.names[i], post));
    }
  });
  ...
  for (const u of game.units) {
    if (u.kind === 'agent') setOrder(game, u, { type: 'hold', zone: zoneAt(map, u).name, point: { x: u.x, y: u.y } });
  }
  return game;
}

function makeAgent(game, team, name, at) {
  return makeUnit(game, {
    team, kind: 'agent', name, x: at.x, y: at.y, r: 0.6, hp: 100, maxHp: 100, speed: 5, reaction: 0.25,
    facing: team === 'attack' ? -Math.PI / 2 : Math.PI / 2, action: 'hold', focusId: null, decision: null,
  });
}
...
export function teamView(game, team) {
  ...
  for (const u of game.units) {
    if (u.team === team) units.push(ownUnit(game, u));
      ...
      units.push({ id: u.id, team: u.team, name: u.name, x: u.x, y: u.y, facing: u.facing, hp: u.hp, maxHp: u.maxHp, r: u.r, alive: true });
    }
  }
  const ghosts = [];
