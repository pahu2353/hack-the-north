...
export const OPPONENT_ACTIONS = ['hold', 'rotate', 'flank', 'retreat', 'retake'];
...
export function applyOpponentPlan(game, plan) {
  for (const order of plan.orders) {
    ...
    const target = zoneByName(game.map, order.zone).center;
...
export function createOpponentCommander({ request = (...args) => fetch(...args), now = () => performance.now() } = {}) {
  ...
  function update(game) {
    ...
    game.botCommander ??= { status: 'waiting', plans: 0, summary: '', model: '', error: '', orders: [] };
    const status = game.botCommander;
    const objective = objectiveKey(game);
    if (pending || (now() < nextAt && objective === lastObjective)) return;
    ...
    return (async () => {
      try {
        ...
        Object.assign(status, {
          ...
          summary: plan.summary, orders: plan.orders, model: data.model,
