...
export function createRenderer(canvas) {
  ...
  function draw(teamView, { pointer, positions } = {}) {
    ...
    // Order lines: where each of your agents has been told to go.
    ...
    for (const u of enemies) drawSoldier({ ...u, ...at(u) }, ENEMY, px);
    for (const u of own) if (u.alive) drawSoldier({ ...u, ...at(u) }, OWN, px, true);
  ...
  function drawSoldier(u, color, px, own = false) {
    ctx.fillStyle = own ? 'rgba(74,163,255,0.12)' : 'rgba(255,93,93,0.12)';
    ctx.beginPath();
    ctx.moveTo(u.x, u.y);
